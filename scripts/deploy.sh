#!/usr/bin/env bash
#
# Build the dashboard locally, then push the built artifact to zima.
#
# zima stays "dumb": it never talks to git, never builds, never installs
...
DRY_RUN=0; SKIP_BUILD=0; CLEAN=0; INSTALL_SERVICE=0; VERIFY=1
...
if git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  if [ -n "$(git status --porcelain)" ]; then
    GIT_DIRTY=" (dirty)"
    warn "working tree has uncommitted changes — deploying anyway"
  fi
  ok "git $GIT_SHA$GIT_DIRTY"
fi
